...
function bindIngrowthValidate(){
 ...
 forbidCommas_ingrowth()

 // multi-stem validation
 multiStemCheck_ingrowth()
...
function forbidCommas_ingrowth(){
  let comments = $('textarea#comment_custom');

  comments.change(function() {
    let commentText = comments.val();
    if (commentText.includes(',')) {
      $('#comma_check_i').modal('show');

      $( "#ok_comma_check_i" ).click(function() {
        comments.val(""); // clear value
        $('#comma_check_i').modal('hide');
      });
    }
  });
}

function multiStemCheck_ingrowth(){
  let multiSelect = $('select#multi_i');
  let mainStemTagInput = $('input#main_stem_tag_i');
