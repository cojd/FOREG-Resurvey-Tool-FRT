...
function queryDB(table, query, params) {
  ...
  var successFn = function (result) {
    ...
    for (var row = 0; row < result.getCount(); row++) {
      ...
      r['hom']        = result.getData(row, "PrevHOM");
      r['psp_tag']    = result.getData(row, "PrevPSPTag");
      ...
      r['gx']         = result.getData(row, "gx");
      r['gy']         = result.getData(row, "gy");
      r['multi']      = result.getData(row, "PrevMulti");
      r['main_stem_tag'] = result.getData(row, "PrevMainStem");
      // records.push(r);
