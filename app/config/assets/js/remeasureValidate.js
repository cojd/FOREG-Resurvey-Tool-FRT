...
function bindRemeasureValidate(){
  ...
  // comments
  forbidCommas_remeasure()

  // multi-stem validation
  multiStemCheck_remeasure()
...
function forbidCommas_remeasure(){
  let comments = $('textarea#comment_custom');

  comments.change(function() {
    let commentText = comments.val();
    if (commentText.includes(',')) {
      $('#comma_check_r').modal('show');

      $( "#ok_comma_check_r" ).click(function() {
        comments.val(""); // clear value
        $('#comma_check_r').modal('hide');
      });
    }
  });
}

function multiStemCheck_remeasure(){
  let multiSelect = $('select#multi_r');
  let mainStemTagInput = $('input#main_stem_tag_r');

  function toggleMainStemTag() {
    if (multiSelect.val() === 'N') {
      mainStemTagInput.prop('disabled', true);
      mainStemTagInput.val('');
    } else {
      mainStemTagInput.prop('disabled', false);
    }
  }
  ...
  // Check when multi-stem value changes
